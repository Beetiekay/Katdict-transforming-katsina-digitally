       ...
       <div>
        ...
        <div className="lg:overflow-hidden overflow-x-scroll md:overflow-hidden">
          <div className='w-[200vw] lg:w-full md:w-full flex gap-4 mt-6 lg:px-10 md:px-5 px-2 mb-6'>
             <button onClick={() => setState(1)} className={`${state === 1 ? " text-green border-2  border-green  " : "outline-noner-2 hover:border-green bg-white rounded-lg text-green"}  text-sm py-4 outline-none px-2 rounded flex-1 cursor-pointer`}>WELCOME ADDRESS</button>
             <button onClick={() => setState(2)} className={`${state === 2 ? " border-2 border-green text-green " : " hover:border-2 hover:border-green bg-white rounded-lg text-green"}  text-sm py-4 px-2 rounded flex-1 cursor-pointer outline-none`}>VISION & MISSION</button>
             <button onClick={() => setState(3)} className={`${state === 3 ? " text-green border-2 border-green py-4 px-2 font-normal" : " py-4 px-2 hover:border-2 hover:border-green bg-white rounded-lg font-normal text-green"}  text-sm font-semibold py-2 px-4 rounded flex-1 cursor-pointer outline-none`}>OUR SERVICES</button>
             <button onClick={() => setState(4)} className={`${state === 4 ? "text-green border-2 border-greenpy-4 font-normal px-2 " : " py-4 px-2 font-normal hover:border-2 hover:border-green bg-white rounded-lg text-green"} outline-none text-sm font-semibold py-2 px-4 rounded flex-1 cursor-pointer`}>OUR PARTNERS</button>
            </div>
        </div>
